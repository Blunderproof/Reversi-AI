    ...
    public int[][] applyMoveToState(int move, int[][] currState){
        ...
        for (incx = -1; incx < 2; incx++) {
            for (incy = -1; incy < 2; incy++) {
                ...
                newState = updateState(newState, row, col, incx, incy, turn); //updated multiple times as the update can be from 1+ directions
            }
        }
...
// call check direction specailial, and take centrow cmovehe value there
    ...
    private int getBestMoveUsingMinMax(RNode parent) {
        ...
        int bestMove = parent.getChildren()[0].getMove();
        double bestScore = getScoreOfBestChild(parent.getChildren()[0], Double.MIN_VALUE);
        
        for (int i = 1; i < parent.getChildren().length; i++) {
            double currentScore = getScoreOfBestChild(parent.getChildren()[i], bestScore);
            // at top level, we know this is going to be max
            if (currentScore > bestScore) {
                bestScore = currentScore;
                bestMove = parent.getChildren()[i].getMove();
            }            
        }                
    ...
    private int getScoreOfBestChild(RNode node, double bestScoreSoFar) {
        ...
        double bestScore = getScoreOfBestChild(node.getChildren()[0], worstScoreForPlayer(node.getPlayer()));
        ...
        for (int i = 1; i < node.getChildren().length; i++) {
            double currentScore = getScoreOfBestChild(node.getChildren()[i], bestScore);
            // at top level, we know this is going to be max
